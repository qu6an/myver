...
export const PlayersManager = () => {
  ...
  const loadPlayers = async () => {
    ...
    try {
      ...
      const playersData: Player[] = profiles.map(profile => {
        ...
        const rating = ratings.find(r => r.user_email === profile.email);
        const playerGames =
  ...
  const filteredPlayers = players.filter(player => {
    const matchesSearch =
      ...
      player.station_name.toLowerCase().includes(searchTerm.toLowerCase());
    ...
    setPlayers(prev =>
      prev.map(player =>
        player.id === playerId
          ? {
              ...player,
              status: player.status === 'ACTIVE' ? 'BANNED' : 'ACTIVE',
            }
          : player
      )
    );
  ...
  const getStatusBadge = (status: string) => {
    if (status === 'ACTIVE') {
      return <Badge className="bg-game-success text-white">Активен</Badge>;
    }
  ...
  return (
    <div className="space-y-6">
          ...
          <Table>
            <TableHeader>
              <TableRow className="border-border">
                ...
                <TableHead className="text-white">Статистика</TableHead>
              ...
              {isLoading ? (
                ...
                paginatedPlayers.map(player => (
                  <TableRow key={player.id} className="border-border">
                    ...
                    <TableCell>
                      <div>
                        <p className="text-sm text-white">{player.city}</p>
                        <p className="text-muted-foreground text-xs">
                          {player.station_name}
                        </p>
                      </div>
                    ...
                    <TableCell>
                      <div>
                        ...
                        {player.lastGame && (
                          <p className="text-muted-foreground text-xs">
                            Последняя:{' '}
                            {new Date(player.lastGame).toLocaleString()}
                          </p>
                        )}
                      </div>
                    </TableCell>
      ...
      <div className="text-muted-foreground text-center text-sm">
        ...
        из {filteredPlayers.length} игроков
      </div>
