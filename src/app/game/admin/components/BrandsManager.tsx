    ...
    <div className="space-y-2">
      ...
      <BrandLogoUploader
        ...
        onLogoUploaded={(logoUrl, logoKey) =>
          setFormData({ ...formData, logo_url: logoUrl, logo_key: logoKey })
        }
      />
    </div>
    ...
    try {
      ...
      const brandsWithCounts = await Promise.all(
        (brandsData || []).map(async brand => {
          ...
          const [lite, easy, normal, hard, lite2, hard2] = await Promise.all([
            supabase
              .schema('game')
              .from('lite_questions')
          ...
          return {
            ...brand,
            questionsCount,
            created_at: new Date(brand.created_at).toLocaleDateString(),
          };
        })
      );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingBrand) {
        ...
        // Add new brand
        const { error } = await supabase.schema('game').from('brands').insert({
          name: formData.name,
          category: formData.category,
          logo_url: formData.logo_url,
          logo_key: formData.logo_key,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          <h2 className="text-2xl font-bold text-white">Управление брендами</h2>
                    ...
                    <TableCell>
                      <div className="flex space-x-2">
                        ...
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => handleDelete(brand.id, brand.logo_key)}
