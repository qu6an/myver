    ...
    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
      ...
      <div className="space-y-2">
        ...
        <Input
          ...
          onChange={e => setFormData({ ...formData, answer_4: e.target.value })}
...
export const QuestionsManager = () => {
  ...
  const categories = [
    ...new Set(
      (currentTab === 'main' ? questions : testQuestions).map(q => q.category)
    ),
  ].filter(Boolean);
  ...
  const deleteAllQuestions = async () => {
    ...
    try {
      ...
      toast.success(`Все ${isTest ? 'тестовые' : 'основные'} вопросы удалены`, {
  ...
  const downloadAllQuestions = async () => {
    try {
      ...
      toast.error('Ошибка при экспорте вопросов', {
        description: error.message || 'Не удалось экспортировать вопросы',
      });
    } finally {
      setIsLoading(false);
    }
  ...
  const syncBrandLogos = async () => {
    try {
      ...
      toast.error('Критическая ошибка синхронизации', {
        description:
          error.message || 'Не удалось синхронизировать логотипы брендов',
      });
    }
  };
  ...
  const loadTestQuestions = async () => {
    try {
      ...
      setTestQuestions(
        (data || []).map((q: any) => ({ ...q, tableName: 'test_questions' }))
      );
    } catch (error) {
  ...
  const handleDelete = async (question: Question) => {
    try {
      const isTest = currentTab === 'test';
      const tableName =
        (question as any).tableName || getTableName(question.type, isTest);
      const { error } = await supabase
        .schema('game')
        .from(tableName as any)
        .delete()
            ...
            <Table>
              <TableHeader>
                <TableRow className="border-border">
                  <TableHead className="text-white">Вопрос</TableHead>
                  <TableHead className="text-white">Бренд</TableHead>
                  <TableHead className="text-white">Категория</TableHead>
                  <TableHead className="text-white">Сложность</TableHead>
                  <TableHead className="text-white">Дата</TableHead>
                  <TableHead className="w-32 text-white">Действия</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {isLoading ? (
