...
export const useGameLifecycle = ({
  ...
  const loadNextQuestion = useCallback(
    async (nextQuestionId: string | null) => {
      ...
      const nextQuestionData = await getQuestionById(nextQuestionId, false); // Fetch brand info first
  ...
  const handleAnswerSelect = async (answerText: string, timeTaken: number) => {
  ...
  const handleShowQuestion = async () => {
    ...
    const fullQuestion = await getQuestionById(currentQuestionData.id, true);
