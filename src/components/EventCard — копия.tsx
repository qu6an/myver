...
const EventCard: React.FC<EventCardProps> = ({
  ...
  return (
    <div
      className={`event-card bg-white rounded-3xl p-6 shadow-lg border border-gray-200 flex flex-col relative overflow-hidden transition-all duration-300 hover:shadow-xl hover:-translate-y-1 border-l-4 ${eventTypeColors[eventType]} ${isHovered ? 'cursor-pointer' : ''}`}
    ...
    >
      ...
      <div className={`event-header flex items-start justify-between mb-4 relative`}>
        <div className={`event-icon w-12 h-12 rounded-xl flex items-center justify-center text-xl ${eventTypeIconColors[eventType]}`}>
          {typeof icon === 'string' ? <i className={icon}></i> : React.createElement(icon)}
        </div>
        <button
        ...
        >
          <i className={`fas ${cardState?.isBookmarked ? 'fa-bookmark text-amber-500' : 'fa-bookmark text-gray-300'}`}></i>
        </button>
      </div>
      
      <h3 className="event-title text-xl font-bold text-gray-900 mb-2">{title}</h3>
      <p className="event-description text-gray-600 mb-5 flex-grow">{description}</p>
      
      <div className="event-meta flex gap-4 mb-5 text-sm text-gray-500 cursor-pointer"
        onClick={(e) => {
          e.stopPropagation();
      ...
      <div className="event-actions flex gap-2.5">
        <button
          className={`btn btn-primary flex-1 py-2.5 px-4 rounded-xl font-bold text-sm cursor-pointer transition-all duration-300 flex items-center justify-center gap-1.5 ${iconColor.replace('text-', 'bg-')} hover:opacity-90`}
          onClick={(e) => {
            e.stopPropagation();
            handlePrimaryClick();
          }}
