...
const formSchema = z
	.object({
		...
		documentIssueDate: z.date({
			required_error: "Дата выдачи обязательна",
		}),
		address: z
			.any()
			.refine((val) => val?.value, { message: "Адрес обязателен" }),
		passportScanPhoto: z.any().refine((file) => file, {
			message: "Скан паспорта с фото обязателен",
		}),
	...
	return (
		<div className='rounded-lg bg-white p-8 shadow-sm'>
			<h1 className='mb-2 text-2xl font-bold'>
				{profile
					? "Редактирование анкеты НДФЛ"
					: "Создание анкеты НДФЛ"}
			</h1>
