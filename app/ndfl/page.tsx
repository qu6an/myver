		...
		const transformedProfile: NdflProfile = {
			...
			address:
				...
				"value" in profile.address
					? {
							value: String(
								(profile.address as { value: unknown }).value
							),
					  }
					: { value: "" },
		};
		...
		<Card className='m-auto mt-10 max-w-2xl'>
			<CardHeader className='text-center'>
				<CardTitle className='text-2xl'>
					Анкета НДФЛ не найдена
				</CardTitle>
			</CardHeader>
			<CardContent className='flex flex-col items-center justify-center text-center'>
				<FilePlus2 className='mb-4 h-16 w-16 text-gray-300' />
				<p className='mb-6 max-w-md text-gray-500'>
					Для подачи сведений вам необходимо создать и заполнить
					анкету. Это займет всего несколько минут.
				</p>
