...
const Sidebar: React.FC = () => {
	...
	const [sidebarItems, _] = useState<NavItem[]>(items);
	const submenuRefs = useRef<{ [key: string]: HTMLUListElement | null }>({});

	// Используем useSyncExternalStore для синхронизации состояния с localStorage
	const isCollapsed = React.useSyncExternalStore(
		...
		// getServerSnapshot
		() => false // Значение по умолчанию для серверного рендеринга
	);

	// Обновляем localStorage при изменении состояния
	const updateLocalStorage = (newCollapsed: boolean) => {
		if (typeof window !== "undefined") {
	...
	const toggleSidebar = () => {
		...
		updateLocalStorage(newCollapsed);
	};
	...
	return (
		<div
			className={`sticky h-[calc(100vh-40px)] transition-all duration-300 ease-in-out flex-col ${
		...
		>
			...
			<div
				className={`sidebar bg-white  p-4 shadow-lg transition-all duration-300 ease-in-out flex flex-col z-20 h-[calc(100vh-40px)] ${
			...
			>
				<div className='logo flex flex-col items-center text-center pb-4 mb-3 border-b border-gray-200 relative transition-all duration-300'>
					<Image
						...
						width={200}
						height={50}
						className={`logo-img w-auto transition-all duration-300 ${
							isCollapsed ? "w-10" : "w-60"
						}`}
					/>
					<button
						className='toggle-sidebar absolute right-3 top-1/2 -translate-y-1/2 bg-indigo-100 border border-indigo-200 rounded-lg w-7 h-7 flex items-center justify-center cursor-pointer text-indigo-60 transition-all duration-300 hover:bg-indigo-20'
						onClick={toggleSidebar}
					>
						<i
							className={`fas fa-chevron-left transition-transform duration-300 ${
				...
				<div
					className={`quick-actions grid grid-cols-2 gap-1.5 mb-4 transition-opacity duration-300 ${
						isCollapsed
				...
				>
					<button
						className='quick-action flex flex-col items-center p-2 bg-indigo-10 rounded-lg border-none cursor-pointer text-gray-600 transition-all duration-300 hover:bg-indigo-200 hover:text-indigo-60 hover:-translate-y-0.5'
					...
					<button
						className='quick-action flex flex-col items-center p-2 bg-indigo-10 rounded-lg border-none cursor-pointer text-gray-600 transition-all duration-300 hover:bg-indigo-20 hover:text-indigo-600 hover:-translate-y-0.5'
				...
				<ul className='nav-links flex-grow overflow-y-auto overflow-x-hidden p-2'>
					...
					<li>
						<a
							href='#'
							className='nav-link flex items-center p-3 text-gray-500 hover:bg-indigo-10 hover:text-indigo-600 hover:translate-x-0.5 rounded-xl transition-all duration-300 font-medium text-sm active:bg-indigo-200 active:text-indigo-70 relative'
						...
						>
							...
							<span
								className={`menu-text transition-all duration-300 whitespace-nowrap overflow-hidden ${
									isCollapsed
										? "opacity-0 w-0 h-0 m-0 p-0"
										: "opacity-100"
								}`}
							>
								ПАНЕЛЬ УПРАВЛЕНИЯ
					...
					{sidebarItems
						...
						.map((item) => (
							...
							>
								...
								>
									<i
										className={`${item.icon} mr-2.5 text-base w-5 text-center flex-shrink-0 transition-all duration-300`}
									></i>
									<span
										className={`menu-text transition-all duration-300 whitespace-nowrap overflow-hidden ${
											isCollapsed
									...
									{item.badge && (
										<span
											className={`menu-badge ml-auto text-xs font-bold rounded-full px-1.5 py-0.5 flex-shrink-0 min-w-[16px] text-center ${
									...
									<span
										className={`submenu-indicator ml-auto transition-transform duration-300 text-xs text-gray-400 flex-shrink-0 ${
								...
								<ul
									...
									className={`submenu list-none overflow-hidden transition-all duration-400 ease-[cubic-bezier(0.4,0,0.2,1)] bg-indigo-100/30 rounded-xl m-0.5 pl-2.5 ${
										activeSubmenu === item.id
											? "max-h-[400px]"
											: "max-h-0"
									}`}
					...
					<li className='nav-title py-3 px-3 text-xs font-bold text-gray-40 uppercase tracking-[0.5px] mt-1 transition-all duration-30'>
						СИСТЕМА
					</li>
				...
				<div className=' pt-4 mt-auto border-t border-gray-200 transition-all duration-300'>
