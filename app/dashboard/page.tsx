...
import Header from "./bestcto/components/dashboard/Header";
...
export default function HomePage() {
	const [originalProjectCardsData] = useState([
		{
			...
			title: "ВИКТОРИНА",
			description:
				"Проверьте свои знания автомобильной тематики, участвуйте в розыгрышах и получайте награды.",
			icon: FaPuzzlePiece,
		...
		},
		{
			...
			title: "EVERYCAR",
			description:
				"Комплексная программа для автосервисов с конкурсами, закупками и бонусами.",
			icon: FaCar,
		...
		},
		{
			...
			description:
				"Участвуйте в главном отраслевом конкурсе, повышайте рейтинг и выигрывайте ценные призы.",
			icon: FaTrophy,
		...
		},
		{
			...
			title: "АКАДЕМИЯ СТО",
			description:
				"Повышайте квалификацию, проходите обучение и получайте сертификаты для вашего СТО.",
		...
		},
		{
			...
			description:
				"Получите официальную сертификацию вашего автосервиса от ведущих производителей.",
			icon: FaAward,
		...
		},
		{
			...
			title: "MYTEAM",
			description:
				"Управляйте командой вашего автосервиса, отслеживайте прогресс и мотивируйте сотрудников.",
			icon: FaUsers,
	...
	const [originalEventCardsData] = useState([
		{
			...
			description:
				"Присоединяйтесь к нашим еженедельным онлайн-воркшопам от ведущих экспертов отрасли!",
			icon: FaWrench,
			iconColor: "text-indigo-600",
			badge: "Еженедельно",
		...
		},
		{
			...
			description:
				"Уникальные образовательные туры с погружением в практику ведущих автосервисов Европы.",
			icon: FaGem,
		...
		},
		{
			...
			description:
				"Главная отраслевая конференция с участием топ-экспертов и производителей.",
			icon: FaCalendar,
			iconColor: "text-emerald-500",
			badge: "Скоро",
	...
	const [filteredProjects, setFilteredProjects] = useState(
		originalProjectCardsData
	);
	const [filteredEvents, setFilteredEvents] = useState(
		originalEventCardsData
	);
	...
	// 	} else {
	// 		const filteredProjects = originalProjectCardsData.filter(
	// 			(project) =>
	// 				project.title
	// 					.toLowerCase()
	...
	// 		const filteredEvents = originalEventCardsData.filter(
	// 			(event) =>
	// 				event.title
	// 					.toLowerCase()
	// 					.includes(searchQuery.toLowerCase()) ||
	...
	return (
		<div className='flex min-h-screen bg-gray-50 font-sans'>
			...
			<div className='main-content flex-1 ml-0 transition-all duration-300'>
				{/* <Header
					title='Главная'
					subtitle='Панель управления вашим успехом'
				/> */}
						...
						<div className='lg:col-span-2'>
							...
							<div className='dashboard-section mb-8'>
								<div className='section-header flex justify-between items-center mb-5'>
									...
									<a
										href='#'
										className='section-link text-indigo-600 font-bold flex items-center gap-2 hover:text-indigo-800 transition-colors'
									>
										Все проекты{" "}
								...
								<div className='projects-grid grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5'>
									{filteredProjects.map((project) => (
										<ProjectCard
											...
											onPrimaryClick={() =>
												console.log(
													`Primary action for ${project.title}`
												)
											}
											onSecondaryClick={() =>
												console.log(
													`Secondary action for ${project.title}`
												)
											}
										/>
									))}
								...
								<div className='events-grid grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5'>
									{filteredEvents.map((event) => (
										<EventCard
											...
											onPrimaryClick={() =>
												console.log(
													`Primary action for ${event.title}`
												)
											}
											onSecondaryClick={() =>
												console.log(
													`Secondary action for ${event.title}`
												)
											}
										/>
									))}
