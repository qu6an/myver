...
export const RatingsManager = () => {
	...
	const loadRatingsData = async (
		...
		try {
			...
			const { data: dailyGamesData, error: dailyGamesError } =
				await supabase
					.schema("game")
					.from("history_games")
					.select("*")
			...
			const formattedDailyRatings = Array.from(dailyRatingsMap.values())
				...
				.map((game, index) => {
					...
					return {
						...
						rank: index + 1,
						gamesPlayed: dailyGamesData.filter(
							(g) => g.email === game.email
						).length,
			...
			const { data: weeklyData, error: weeklyError } =
				await weeklyQuery.order("points", { ascending: false });
			...
			const processedWinners = winnersData
				.map((record) => {
					...
					const populatedWinners = winners.map((winner: any) => ({
						...winner,
						name:
							profileMap.get(winner.email)?.first_name ||
							"Неизвестно",
						nickname:
							profileMap.get(winner.email)?.nick_name_game ||
							"player",
					}));
					return {
						...record,
	...
	const handleExportRatings = async (format: "csv" | "excel" = "csv") => {
		try {
			...
			const exportData = ratingsData.map((rating, index) => ({
				Место: index + 1,
				Имя:
					`${rating.name || ""} ${rating.surname || ""}`.trim() ||
					"Неизвестно",
							...
							<div className='flex items-end gap-4'>
								<div className='flex-1'>
									<Label className='text-white'>
										Неделя для розыгрыша
									</Label>
									<Select
										value={lotteryWeek}
										onValueChange={setLotteryWeek}
									>
				...
				<div className='space-y-4 text-center'>
					...
					<div className='text-muted-foreground grid grid-cols-1 gap-4 text-sm md:grid-cols-3'>
						...
						<div>
							...
							<p>
								1 игрок, выбирается случайным образом по билетам
							</p>
						</div>
						<div>
							<p className='mb-1 font-medium text-white'>
							...
							<p>
								После определения призеров все очки обнуляются
							</p>
						</div>
					</div>
				</div>
